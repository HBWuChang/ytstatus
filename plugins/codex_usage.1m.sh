...
make_icon() {
  local pct5="${1:-0}" pct7="${2:-0}" mode="${3:-bars}"
  python_eval "$pct5" "$pct7" "$OPENAI_ICON" "$mode" <<'PY'
...
if mode == "logo":
    print(base64.b64encode(png(LOGO_W,LOGO_H,logo)).decode())
    sys.exit(0)
...
HAS_5H=false
HAS_7D=false
if is_pct "$PCT_5H"; then
  HAS_5H=true
else
  PCT_5H=0
fi
if is_pct "$PCT_7D"; then
  HAS_7D=true
else
  PCT_7D=0
fi

if [ "$HAS_5H" != "true" ] && [ "$HAS_7D" != "true" ]; then
  show_error "No Codex usage windows reported."
fi

COLOR_5H=""
COLOR_7D=""
[ "$HAS_5H" = "true" ] && COLOR_5H="$(color_for_pct "$PCT_5H")"
[ "$HAS_7D" = "true" ] && COLOR_7D="$(color_for_pct "$PCT_7D")"
title_color() {
  local c1="$1" c2="$2"
...
if [ "$HAS_5H" = "true" ] && [ "$HAS_7D" = "true" ] && [ "$SHOW_7D" = "true" ]; then
  TITLE_COLOR="$(title_color "$COLOR_5H" "$COLOR_7D")"
  TITLE="${PCT_5H}%/${PCT_7D}%"
elif [ "$HAS_5H" = "true" ]; then
  TITLE_COLOR="$COLOR_5H"
  TITLE="${PCT_5H}%"
else
  TITLE_COLOR="$COLOR_7D"
  TITLE="${PCT_7D}%"
fi

render_text_title() {
  local text_icon
  text_icon="$(make_icon 0 0 logo 2>/dev/null)"
  [ -n "$text_icon" ] || text_icon="$OPENAI_ICON"
  if [ -n "$TITLE_COLOR" ]; then
    echo "${TITLE} | templateImage=${text_icon} color=${TITLE_COLOR}"
  else
    echo "${TITLE} | templateImage=${text_icon}"
  fi
}

if [ "$SHOW_BARS" = "true" ] && [ "$HAS_5H" = "true" ] && [ "$HAS_7D" = "true" ]; then
...
if [ "$HAS_5H" = "true" ]; then
  ...
  echo "5h window: Not currently reported | color=#888888"
fi
...
if [ "$HAS_7D" = "true" ]; then
  ...
  echo "Weekly window: Not currently reported | color=#888888"
fi
